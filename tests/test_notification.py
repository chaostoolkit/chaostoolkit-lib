...
def test_notify_to_http_endpoint():
    ...
    with requests_mock.mock() as m:
        ...
        notify(
            {"notifications": [{"type": "http", "url": "http://example.com"}]},
...
def test_notify_to_http_endpoint_can_timeout(logger):
    ...
    with requests_mock.mock() as m:
        ...
        notify(
            {"notifications": [{"type": "http", "url": url}]}, RunFlowEvent.RunStarted
        )
...
def test_notify_to_http_endpoint_may_fail(logger):
    ...
    with requests_mock.mock() as m:
        ...
        m = "Notification sent to '{u}' failed with '{t}'".format(u=url, t="boom")
        logger.debug.assert_called_with(m)
